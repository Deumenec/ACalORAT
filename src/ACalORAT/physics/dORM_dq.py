...
class dORM_dq(BaseIntegrals):
    ...
    def dRij_dqk_thick23(self, Ei : Elements, Ej : Elements, Ek : Elements):
        ...
        #Terms for the thick quadrupole formula if they have or not quadrupole component
        
        if hasattr(Ek, "K") and np.abs(Ek.K[0]) >1e-9:
            Ik0  = self.Ik0(Ek)
            Iks2 = self.Iks2(Ek)
            Ikc2 = self.Ikc2(Ek)
        else: 
        ...
        #Choose integral along correctors depening on if they have quadrupole inside or not
    ...
    def dRij_dqk_thick23_disp(self, Ei : Elements, Ej : Elements, Ek : Elements):
        ...
        dnj_dqk = self.dni_dqk_integral(Ej, Ek)  
