...
def disp_i_dk(ring, quad):
    """
    For a given bend, we calculate the derivative of dispersion in bpms in the 
...
de_dk = np.zeros((len(ind["dip"])))
dx_dk = np.zeros((len(ind["dip"]), len(ind["sex"])))
dxp_dk = np.zeros((len(ind["dip"]), len(ind["sex"])))

if compute:
    for i in range(len(ind["dip"])):
        di_dk[i], de_dk[i], dx_dk[i], dxp_dk[i] = disp_i_dk(ring, ind["dip"][i])
...
i = cORM.dxsex_ddip(cORM.bpm, cORM.dip)
j = cORM.dxpsex_ddip(cORM.bpm, cORM.dip)
...
f1 = di_dk_no_sext#-di_dk_energy_term
